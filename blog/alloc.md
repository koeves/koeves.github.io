...
#define  ALIGN(_size) (_size % 8 > 0) ? (_size + (ALIGN_SIZE - (_size % ALIGN_SIZE)) + BLOCK_SIZE) : (_size + BLOCK_SIZE);  
```
 ...
 */
static struct block *get_block(void *ptr) 
{
...
void *malloc(size_t size)
{
    ...
    curr->free = 0;
    curr->size = aligned_size - BLOCK_SIZE;
    curr->next_free = NULL;
...
void free(void *ptr)
{  
    struct block *curr = get_block(ptr);
    curr->free = 1;
...
struct block *get_free_block(size_t size)
{
    ...
    while (curr) {
        ...
        if ((curr->size >= size) && (current->free))
            return curr;
...
Second, if we find a suitable block to reuse, we need to do some maintaining on our freelist. When we have a singly-linked list of free blocks, what scenarios might occur when we try to unlink a block?
...
```c
static void remove_block(struct block *curr)
{
    if (free_head == free_tail) {
        free_head = free_tail = NULL;
        curr->next_free = NULL;
    }
    else if (free_head == curr) {
        free_head = free_head->next_free;
        curr->next_free = NULL;
    }
    else if (free_tail == curr) {
...
void *malloc(size_t size)
{
    struct block *curr = get_free_block(size);
    ...
    if (current) {
        ...
        if ((curr = sbrk(aligned_size)) == (void *)-1) 
            perror("sbrk");
